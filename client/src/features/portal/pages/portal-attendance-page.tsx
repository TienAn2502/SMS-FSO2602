...
import {
    ...
    initializePortalAttendanceRecords,
...
export function PortalAttendancePage() {
    ...
    useEffect(() => {
        if (sessionQuery.data?.records) {
            // build data từ BE trả về thành cấu trúc bản nháp
    ...
    const createMutation = useMutation({
        mutationFn: async (input: {
            ...
            return initializePortalAttendanceRecords(session.id);
        },
        onSuccess: (session) => {
            setActiveSessionId(session.id);
            setDraft(buildRecordDraft(session.records));
            queryClient.setQueryData(
                ['portal', 'attendance-sessions', session.id],
                session,
            );
        ...
        onError: (error) => {
            ...
            toast.error(
                getErrorMessage(
                    apiError?.code,
                    apiError?.message ?? 'Tạo phiên thất bại',
                ),
            );
        },
    });
    ...
    const reloadStudentsMutation = useMutation({
        mutationFn: () => initializePortalAttendanceRecords(activeSessionId!),
        onSuccess: (data) => {
            setDraft(buildRecordDraft(data.records));
            queryClient.setQueryData(
                ['portal', 'attendance-sessions', activeSessionId],
                data,
            );
        ...
        onError: (error) => {
            ...
            toast.error(
                getErrorMessage(
                    apiError?.code,
                    apiError?.message ?? 'Nạp danh sách thất bại',
                ),
            );
        },
    });
    ...
    const saveMutation = useMutation({
        mutationFn: () => {
            const records = recordsToBulkPayload(draft);

            console.info('[Portal attendance] Saving attendance records', {
                sessionId: activeSessionId,
                records,
            });

            return bulkUpsertPortalAttendanceRecords(activeSessionId!, {
                records,
            });
        },
        onSuccess: (data) => {
    ...
    const closeMutation = useMutation({
        ...
        onError: (error) => {
            ...
            toast.error(
                getErrorMessage(
                    apiError?.code,
                    apiError?.message ?? 'Đóng phiên thất bại',
                ),
            );
        },
    });
        ...
        <div className='space-y-6'>
            <div>
                ...
                <p className='text-sm text-muted-foreground'>
                    Mở phiên theo lớp môn được phân công — hệ thống tự nạp học
                    sinh ghi danh ACTIVE.
                </p>
            </div>
                    ...
                    <CardContent className='grid gap-4 md:grid-cols-2'>
                        <div className='space-y-1 md:col-span-2'>
                            ...
                            <select
                                ...
                                onChange={(event) =>
                                    setCourseSectionId(event.target.value)
                                }
                            >
                        ...
                        <div className='space-y-1'>
                            ...
                            <Input
                                ...
                                value={periodNumber}
                                onChange={(event) =>
                                    setPeriodNumber(Number(event.target.value))
                                }
                            />
                        </div>
                        <div className='md:col-span-2'>
                            <Button
                                disabled={
                                    !courseSectionId || createMutation.isPending
                                }
                                onClick={() =>
            ...
            {session ? (
                <Card>
                    <CardHeader className='flex flex-row flex-wrap items-center justify-between gap-2'>
                        <div>
                            <CardTitle className='text-base'>
                                {session.courseSectionCode} —{' '}
                                {formatDateVi(session.sessionDate)} (tiết{' '}
                                {session.periodNumber})
                            </CardTitle>
                    ...
                    <CardContent>
                        ...
                        <AttendanceRecordGrid
                            records={session.records} // dữ liệu từ BE
                            draft={draft} // bản nháp để
                            readonly={isClosed}
                            ...
                            onDraftChange={(studentId, patch) => {
                                // Thay đổi dữ liệu trong bản nháp để hiển thị lên UI
                                setDraft((current) => {
                                    ...
                                    return {
                                        ...current, // dữ lại thông tin của HS khác
                                        [studentId]: { ...existing, ...patch }, // thay đổi thông tin của HS đó
                                    };
                                });
                            }}
                        />
