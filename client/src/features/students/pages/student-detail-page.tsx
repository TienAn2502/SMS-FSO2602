...
export function StudentDetailPage() {
    ...
    const enrollmentsQuery = useQuery({
        ...
        queryFn: () =>
            fetchStudentEnrollments(id, { limit: 50, sortOrder: 'desc' }),
        enabled: Boolean(id),
    ...
    const enrollmentColumns = useMemo<ColumnDef<StudentEnrollment>[]>(
        () => [
            ...
            },
            {
                ...
                header: 'Lớp HC',
                cell: ({ row }) =>
                    `${row.original.homeroomClassCode} — ${row.original.homeroomClassName}`,
            },
            {
    ...
    const activeEnrollment =
        enrollments.find((e) => e.status === 'ACTIVE' && e.semesterIsCurrent) ??
        enrollments.find((e) => e.status === 'ACTIVE') ??
        (student.currentEnrollment
            ? (enrollments.find(
                  (e) => e.id === student.currentEnrollment?.id,
              ) ?? null)
            : null);
        ...
        <div className='space-y-6'>
            <div className='flex flex-wrap items-center justify-between gap-4'>
                <div>
                    <Button
                        variant='ghost'
                        size='sm'
                        render={<Link to={ROUTES.students} />}
                    >
                        ← Danh sách học sinh
                    </Button>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Thông tin PH gắn với học sinh này
                    </CardDescription>
                </CardHeader>
                <CardContent>
