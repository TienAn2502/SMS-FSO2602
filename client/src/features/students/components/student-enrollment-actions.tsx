...
export function StudentEnrollmentActions({
    ...
    const enrollMutation = useMutation({
        ...
        onError: (error) => {
            ...
            toast.error(
                getErrorMessage(
                    apiError?.code,
                    apiError?.message ?? 'Ghi danh thất bại',
                ),
            );
        },
    });
    ...
    const transferMutation = useMutation({
        ...
        onError: (error) => {
            ...
            toast.error(
                getErrorMessage(
                    apiError?.code,
                    apiError?.message ?? 'Chuyển lớp thất bại',
                ),
            );
        },
    });
    ...
    const withdrawMutation = useMutation({
        ...
        onError: (error) => {
            ...
            toast.error(
                getErrorMessage(
                    apiError?.code,
                    apiError?.message ?? 'Rút lớp thất bại',
                ),
            );
        },
    });
    ...
    const activeSemesterIds = useMemo(
        () =>
            new Set(
                activeEnrollments.map((enrollment) => enrollment.semesterId),
            ),
        [activeEnrollments],
    );
    ...
    const enrollableSemesters = useMemo(
        () =>
            enrollSemesters.filter(
                (semester) => !activeSemesterIds.has(semester.id),
            ),
    ...
    return (
        <>
            <div className='flex flex-wrap gap-2'>
                {canEnroll && (
                    <Button onClick={() => setSheetMode('enroll')}>
                        Ghi danh lớp
                    </Button>
                )}
            ...
            {activeEnrollment ? (
                <p className='mt-2 text-sm text-muted-foreground'>
                    ...
                    {activeEnrollment.semesterName}). Muốn đổi lớp trong cùng
                    học kỳ, dùng{' '}
                ...
                <SheetContent>
                    ...
                    >
                        ...
                        <div className='space-y-2'>
                            ...
                            {enrollYearId &&
                            enrollableSemesters.length === 0 ? (
                                <p className='text-sm text-muted-foreground'>
                                    Học sinh đã ghi danh tất cả học kỳ trong năm
                                    học này.
                                </p>
                            ) : null}
                        </div>
            ...
            >
                <SheetContent>
                    <SheetHeader>
                        ...
                        <SheetDescription>
                            Chuyển từ {activeEnrollment?.homeroomClassCode} sang
                            lớp khác cùng học kỳ
                        </SheetDescription>
                    </SheetHeader>
                    ...
                    >
                        ...
                        <div className='space-y-2'>
                            ...
                            <Input
                                id='transferredAt'
                                type='date'
                                {...transferForm.register('transferredAt')}
                            />
                        </div>
            ...
            >
                <SheetContent>
                    <SheetHeader>
                        ...
                        <SheetDescription>
                            Học sinh sẽ chuyển sang trạng thái đã rút khỏi lớp
                            hiện tại
                        </SheetDescription>
                    </SheetHeader>
