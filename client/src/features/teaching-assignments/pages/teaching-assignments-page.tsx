...
export function TeachingAssignmentsPage() {
    ...
    const teacherFilter = getColumnFilterValue<string>(
        columnFilters,
        'teacherId',
    );
    ...
    useEffect(() => {
        ...
        setColumnFilters((prev) => {
            const hasSemester = prev.some(
                (filter) => filter.id === 'semesterId',
            );
    ...
    const createMutation = useMutation({
        ...
        onError: (error) => {
            ...
            toast.error(
                getErrorMessage(
                    apiError?.code,
                    apiError?.message ?? 'Phân công thất bại',
                ),
            );
        },
    });
    ...
    const statusMutation = useMutation({
        mutationFn: (id: string) =>
            updateTeachingAssignmentStatus(id, 'INACTIVE'),
        onSuccess: () => {
            void queryClient.invalidateQueries({
                queryKey: ['teaching-assignments'],
            });
