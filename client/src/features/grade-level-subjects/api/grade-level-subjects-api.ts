...
export async function fetchGradeLevelSubjectsForSystemAdmin(
    params: ListGradeLevelSubjectsParams = {},
) {
    const { data } = await api.get<ApiPaginatedResponse<GradeLevelSubject>>(
        '/grade-level-subjects/system-admin',
        { params },
    );
    return { items: data.data, meta: data.meta };
}
...
export interface GradeLevelSubjectUpdate {
    id: string;
    periodsPerYear?: number | null;
    isRequired?: boolean;
    evaluationMode?: SubjectEvaluationMode;
...
export async function updateGradeLevelSubjects(
    updates: GradeLevelSubjectUpdate[],
): Promise<GradeLevelSubject[]> {
    const { data } = await api.patch<ApiSuccessResponse<GradeLevelSubject[]>>(
        '/grade-level-subjects/batch',
        { updates },
    );
    return data.data;
}

export async function fetchAllGradeLevelSubjects(
    gradeLevelId?: string,
    page: number = 1,
) {
...
export function getThptBgdEvaluationModeReference(
    ...
    return THPT_BGD_PASS_FAIL_SUBJECTS.has(subjectCode)
        ? 'PASS_FAIL'
        : 'NUMERIC';
}
...
export function getThptBgdTotalPeriodsReference(
    subjectCode: string,
): number | null {
