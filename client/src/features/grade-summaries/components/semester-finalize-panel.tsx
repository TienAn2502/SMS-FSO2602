...
export function SemesterFinalizePanel({
  ...
  if (readinessQuery.isLoading) {
    return (
      <p className='text-sm text-muted-foreground'>Đang kiểm tra điều kiện khóa học kỳ…</p>
    );
  }
  ...
  return (
    <div className='space-y-3 rounded-lg border bg-muted/30 p-4'>
      <div className='flex flex-wrap items-center justify-between gap-2'>
        <div>
          <p className='text-sm font-medium'>Khóa học kỳ toàn trường</p>
        ...
        <div className='flex flex-wrap gap-2'>
          ...
          <Button
            type='button'
            size='sm'
            disabled={Boolean(finalizeDisabledReason) || finalizeAllMutation.isPending}
                  ...
                  <li key={`${issue.code}-${index}`}>
                    ...
                    {issue.courseSectionCodes?.length ? (
                      <span className='block text-xs'>
                        Lớp môn: {issue.courseSectionCodes.join(', ')}
                      </span>
                    ) : null}
                  </li>
                ))}
