...
export function GradeSummariesPage() {
    ...
    const promotionFinalizeState = useMemo(() => {
        ...
        const hasHk2 = yearSemesters.some(
            (semester) => semester.code === 'HK2',
        );
        if (!hasHk2) {
            return {
                disabled: true,
        ...
        const currentSemester = yearSemesters.find(
    ...
    const recomputeYearMutation = useMutation({
        ...
        onSuccess: (data) => {
            toast.success(
                `Đã tái tính tổng kết năm: ${data.yearSummariesUpserted} học sinh`,
            );
            void queryClient.invalidateQueries({
                queryKey: ['grade-summaries'],
            });
        ...
        onError: (error) => {
            ...
            toast.error(
                getErrorMessage(
                    apiError?.code,
                    apiError?.message ?? 'Tái tính năm thất bại',
                ),
            );
        },
    });
    ...
    const selectedSemesterName = useMemo(
        () =>
            filterSemesters.find((semester) => semester.id === semesterFilter)
                ?.name,
                        ...
                        <div className='space-y-2'>
                            <div className='flex flex-wrap gap-2'>
                                ...
                                <Button
                                    ...
                                    disabled={
                                        !yearFilter ||
                                        recomputeYearMutation.isPending
                                    }
                                    onClick={() =>
                                        recomputeYearMutation.mutate()
                                    }
                                >
                                    Tái tính năm
                                </Button>
