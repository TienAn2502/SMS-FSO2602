...
export function ClassPlacementPage() {
    ...
    const assignMutation = useMutation({
        ...
        onError: (error) => {
            ...
            toast.error(
                getErrorMessage(
                    apiError?.code,
                    apiError?.message ?? 'Xếp lớp thất bại',
                ),
            );
        },
    });
    ...
    const autoMutation = useMutation({
        mutationFn: () =>
            autoBalanceClassPlacement({
                ...
                gradeLevelId,
                reason: reason || undefined,
                studentIds: selectedIds.size > 0 ? [...selectedIds] : undefined,
            }),
        onSuccess: (data) => {
            clearSelection();
            invalidate();
    ...
    const allSelectedOnPage =
        items.length > 0 &&
        items.every((row) => selectedIds.has(row.studentId));
    ...
    return (
        <div className='space-y-6'>
            <div className='flex flex-wrap items-start justify-between gap-4'>
                <div>
                    ...
                    <p className='text-sm text-muted-foreground'>
                        HS chưa có lớp trong học kỳ: ở lại lớp hoặc mới lên cấp.
                        Xếp tay, chia đều theo khối, hoặc import Excel (mỗi
                        sheet một lớp).
                    </p>
                </div>
                <Button
            ...
            <div className='grid gap-3 sm:grid-cols-2 lg:grid-cols-4'>
                ...
                <div className='space-y-1.5'>
                    ...
                    <select
                        ...
                        onChange={(event) => {
                            setReason(
                                event.target.value as '' | PlacementReason,
                            );
                            setPage(1);
                            clearSelection();
                        }}
            ...
            <div className='space-y-3 rounded-lg border bg-muted/30 p-4'>
                ...
                <p className='text-sm text-muted-foreground'>
                    HS ở lại chỉ được xếp vào lớp cùng khối năm trước (vd. ở lại
                    12 → chỉ lớp khối 12).
                </p>
                {retainedGradeConstraint.hasConflict ? (
                    <p className='text-sm text-amber-700 dark:text-amber-400'>
                        Đang chọn HS ở lại thuộc nhiều khối (
                        {retainedGradeConstraint.codes.join(', ')}). Hãy xếp
                        theo từng khối.
                    </p>
                ) : null}
            ...
            <div className='space-y-3 rounded-lg border bg-muted/30 p-4'>
                ...
                <p className='text-sm text-muted-foreground'>
                    Chọn khối trước. Nếu có HS được chọn thì chỉ chia các HS đó;
                    không chọn = toàn bộ danh sách lọc hiện tại (theo loại).
                </p>
                {preview ? (
                    <ul className='grid gap-1 text-sm text-muted-foreground sm:grid-cols-2'>
                        ...
                        <li>
                            Không xếp được (hết chỗ): {preview.unplacedCount}
                        </li>
                ...
                <AlertDialog>
                    ...
                    <AlertDialogContent>
                        <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                                Hệ thống sẽ gán HS vào các lớp ACTIVE của khối
                                đã chọn, ưu tiên lớp ít HS hơn và tôn trọng sức
                                chứa. Thao tác tạo ghi danh ACTIVE.
                            </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
