...
function parseImportErrors(error: unknown): StudentImportRowError[] {
...
export function StudentImportSheet({
  ...
  const [importErrors, setImportErrors] = useState<StudentImportRowError[]>([]);
  ...
  useEffect(() => {
    if (!open || !semesters.length) {
      return;
    }

    const currentSemester = semesters.find((semester) => semester.isCurrent);
    setSemesterId(currentSemester?.id ?? semesters[0]?.id ?? '');
  }, [open, semesters]);
  ...
  const templateMutation = useMutation({
    mutationFn: () => downloadStudentsImportTemplate(academicYearId || undefined),
    onSuccess: () => {
  ...
  const importMutation = useMutation({
    ...
    onSuccess: (result) => {
      setImportErrors([]);
      toast.success(
        `Import thành công ${result.successCount} học sinh (${result.created} mới, ${result.updated} cập nhật)`,
      );
      onSuccess();
      onOpenChange(false);
    },
  ...
  const handleImport = () => {
    ...
    if (!academicYearId || !semesterId) {
      toast.error('Vui lòng chọn năm học và học kỳ ghi danh');
      return;
    }
      ...
      <SheetContent className='overflow-y-auto sm:max-w-lg'>
        ...
        <div className='space-y-4 px-4'>
          <div className='space-y-2'>
            <Label htmlFor='import-year'>Năm học ghi danh</Label>
            <select
              id='import-year'
          ...
          <div className='space-y-2'>
            <Label htmlFor='import-semester'>Học kỳ ghi danh</Label>
            <select
              id='import-semester'
              className={selectClassName}
              value={semesterId}
