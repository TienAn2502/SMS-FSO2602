...
function useFileUpload(options: UploadOptions) {
    ...
    const uploadFile = async (
        ...
        try {
            ...
            const rawResult = await options.upload(
                file,
            ...
            const result = normalizeImageUploadResult(rawResult);
            if (!result.src) throw new Error('Upload failed: No URL returned');
    ...
    const uploadFiles = async (files: File[]): Promise<ImageUploadResult[]> => {
        ...
        return results.filter(
            (item): item is ImageUploadResult => item !== null,
        );
    };
...
export const ImageUploadNode: React.FC<NodeViewProps> = (props) => {
    ...
    const handleUpload = async (files: File[]) => {
        // 1. Upload tất cả file, nhận kết quả
        const results = await uploadFiles(files);

        if (results.length > 0) {
            // 2. Lấy vị trí của node upload trong editor
            ...
            if (isValidPosition(pos)) {
                ...
                // 3. Build mảng image nodes từ kết quả upload
                const imageNodes = results.map((result, index) => {
                    ...
                    return {
                        type: imageType,
                        attrs: {
                            src: result.src, // signed URL từ R2
                            alt: filename,
                            title: filename,
                            fileId: result.fileId ?? null,
                            temp: Boolean(result.temp),
                ...
                // 4. Thay thế node upload bằng image node thực sự
                props.editor
                    .chain()
                    .focus()
                ...
                // 5. Di chuyển con trỏ sang node tiếp theo
